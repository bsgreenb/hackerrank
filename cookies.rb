...
def cookies(k, arr)
    ...
    while minHeap.peek_min && minHeap.peek_min.value < k
        return -1 unless minHeap.heap_arr[2]

        puts "HI"
        lowest_val = minHeap.pop_min.value
        second_lowest_val = minHeap.pop_min.value

        new_val = lowest_val + (2 * second_lowest_val)
        minHeap.insert(new_val)
        puts "YO"
        # puts minHeap.heap_arr[1..-1].map(&:value).inspect
        numOps += 1
    end

    return -1 if !minHeap.peek_min
